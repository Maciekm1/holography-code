...
if __name__ == '__main__':
    ...
    layout = [
        ...
        [sg.Text('Select Background Image',
                 size=(35, 1)),
         # sg.In(default_text='Select a video'),
         sg.FileBrowse(initial_folder='', key='-BG_IMAGE-')],
        [sg.Text('Propagator Scheme', size=(35, 1)), sg.Radio('Rayleigh-Sommerfeld', 'SCHEME', key='-RS-'),
         sg.Radio('Modified', 'SCHEME', key='-MOD-')],
        [sg.Text('Refraction index of media (water = 1.3226)', size=(35, 1)),
         sg.InputText(default_text=1.3226, key='-N-')],
        ...
        [sg.Text('Gradient Stack Threshold (~0.1) just for RS', size=(35, 1)),
         sg.InputText(default_text=0.1, key='-THRESHOLD-')],
    ...
    while True:
        ...
        if event == 'Cancel':  # or event == sg.WIN_CLOSED:
            ...
            BG_PATH.append(values['BG_IMAGE'])
    ...
    for k in range(len(PATH)):
        ...
        BG_IMAGE = f.bgPathToArray(BG_IMAGE)
        ...
        if scheme[k] == '-RS-':
            for _ in tqdm(pool.imap_unordered(f.positions_batch,
                                              zip(IT, MED, n, lam, mpp, sz, numsteps, threshold, pmd, BG_IMAGE)),
                          total=NUM_FRAMES):
                results.append(_)

        elif scheme[k] == '-MOD-':
            for _ in tqdm(pool.imap_unordered(f.positions_batch_modified,
                                              zip(IT, MED, n, lam, mpp, sz, numsteps, threshold, pmd)),
                          total=NUM_FRAMES):
                results.append(_)
        ...
        if export[k]:
            ...
            if scheme[k] == '-RS-':
                expath = path + '/' + pp + '_TH' + str(THRESHOLD[k]) + '_PMD' + str(PMD[k]) + '_SZ' + str(
                    SZ[k]) + '_NUMSTEPS' + str(NUMSTEPS[k]) + '_RS' + '.csv'
                POSITIONS.to_csv(expath)

            elif scheme[k] == '-MOD-':
                expath = path + '/' + pp + '_TH' + str(THRESHOLD[k]) + '_PMD' + str(PMD[k]) + '_SZ' + str(
                    SZ[k]) + '_NUMSTEPS' + str(NUMSTEPS[k]) + '_MOD' + '.csv'
                POSITIONS.to_csv(expath)
            print('Exported to: \n', expath)
            print(
                '---------------------------------------------------------------------------------------------------------------------------------------------------')
