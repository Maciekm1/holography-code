...
def clean_tracks_search_sphere(track, rsphere):
    ...
    x, y, z, t, fr = track['X'].values, track['Y'].values, track['Z'].values, track['TIME'].values, track[
        'FRAME'].values
    ...
    if num_particles == 1:
        ...
        for n in range(num_particles):
            ...
            for i in range(len(dd)):
                ...
                if i == 0:
                    x0, y0, z0, i_fs0, i_gs0, fr0, t0 = dd['X'][i], dd['Y'][i], dd['Z'][i], dd['I_FS'][i], dd['I_GS'][
                        i], dd['FRAME'][i], dd['TIME'][i]
                    track = [(x0, y0, z0, i_fs0, i_gs0, fr0, t0, particle + 0.1 * (n + 1))]
                    idtrack.append(i)


                else:
                    x, y, z, i_fs, i_gs, fr, t = dd['X'][i], dd['Y'][i], dd['Z'][i], dd['I_FS'][i], dd['I_GS'][i], \
                    dd['FRAME'][i], dd['TIME'][i]
                    dist = np.sqrt((x - x0) ** 2 + (y - y0) ** 2 + (z - z0) ** 2)
