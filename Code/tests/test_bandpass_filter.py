...
def test_bandpass_filter_valid_input():
    ...
    assert isinstance(filtered_image, np.ndarray), "Filtered image should be a numpy array"
    assert isinstance(bandpass_filter_array, np.ndarray), "Bandpass filter array should be a numpy array"
    assert filtered_image.shape == image.shape, "Filtered image should have the same shape as input image"
    assert bandpass_filter_array.shape == image.shape, "Bandpass filter array should have the same shape as input image"
...
def test_bandpass_filter_edge_cases():
    ...
    assert np.all(filtered_image == 0), "Filtered image should be zero for a zero input image"
    assert np.all(bandpass_filter_array >= 0), "Bandpass filter should not contain negative values"
    ...
    assert filtered_image.shape == image.shape, "Output shape mismatch for extreme cutoff sizes"
    assert bandpass_filter_array.shape == image.shape, "Bandpass filter shape mismatch for extreme cutoff sizes"
